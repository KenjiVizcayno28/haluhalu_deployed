...
class SendMessageView(LoginRequiredMixin, CreateView):
    ...
    def get_initial(self):
        ...
        # New logic to handle messaging a product seller
        product_pk = self.kwargs.get('product_pk')
        if product_pk:
            product = get_object_or_404(Product, pk=product_pk)
            # Hardcode the recipient to be the product's seller
            initial['receiver'] = product.seller
            # Pre-fill the subject line with the product's title
            initial['subject'] = f"Regarding your product: {product.title}"
        else:
            # Existing logic for replying to a message
